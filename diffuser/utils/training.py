    ...
    def train_epoch(self, n_train_steps, epoch=0):        
        ...
        for step in range(n_train_steps):
            for _ in range(self.gradient_accumulate_every):
                ...
                if cond is not None:
                    noise = apply_conditioning(noise, cond, self.model.action_dim, self.model.goal_dim)     # Apply conditioning
                ...
                noisy_sample = self.noise_scheduler.add_noise(sample, noise, timesteps)
                
                noise_pred = self.model(sample=noisy_sample, timestep=timesteps, condition=cond)
